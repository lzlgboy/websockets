...
class Frame(NamedTuple):
    ...
    def parse(
        cls,
        readexactly: Callable[[int], Generator[None, None, bytes]],
        *,
        mask: bool,
        ...
        :param readexactly: generator-based coroutine that reads exactly the
            requested number of bytes or raises an exception if the end of file
            is reached
        ...
        if mask:
            mask_bits = yield from readexactly(4)

        # Read the data.
        data = yield from readexactly(length)
        if mask:
